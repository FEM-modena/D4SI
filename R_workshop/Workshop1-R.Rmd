...
`<-` è l'operatore di assegnazione. Assegna valori sulla destra a oggetti sulla
sinistra. Così, dopo aver eseguito `x <- 3`, il valore di `x` è `3`. La freccia sembra anche una bocca (con la lingua), il che la rende facile da pronunciare come `x` **mangia** 3. Per ragioni storiche, puoi anche usare `=`
per le assegnazioni, ma non in ogni contesto. A causa del
[lieve](https://blog.revolutionanalytics.com/2008/12/use-equals-or-arrow-for-assignment.html)
[differenze](https://r.789695.n4.nabble.com/Is-there-any-difference-between-and-tp878594p878598.html)
nella sintassi, è buona pratica usare sempre `<-` per le assegnazioni. 

In RStudio, digitando <kbd>Alt</kbd> + <kbd>-</kbd> (premendo <kbd>Alt</kbd> allo
contemporaneamente al tasto <kbd>-</kbd>) scriverà ` <- ` in un singolo colpo di tasto in un PC, mentre digitando <kbd>Option</kbd> + <kbd>-</kbd> (premere <kbd>Option</kbd> allo
contemporaneamente al tasto <kbd>-</kbd>) fa lo stesso in un Mac.
...
* R è sensibile alle maiuscole e alle minuscole, quindi, per esempio, `weight_kg` è diverso da `Weight_kg`.
* Ci sono alcuni nomi che non possono essere usati perché sono i nomi di funzioni fondamentali in R (per esempio, `if`, `else`, `for`, vedi
[qui](https://stat.ethz.ch/R-manual/R-devel/library/base/html/Reserved.html)
per una lista completa). In generale, anche se è permesso, è meglio non usare
altri nomi di funzioni (ad esempio, `c`, `T`, `mean`, `data`, `df`, `weights`). Se sei in
dubbio, controlla l'aiuto per vedere se il nome è già in uso. 
...
Fan](https://jef.works/R-style-guide/) e da
tidyverse](https://style.tidyverse.org/). Lo stile tidyverse è molto completo e può sembrare travolgente all'inizio. Puoi installare il pacchetto [**`lintr`**](https://github.com/jimhester/lintr) per controllare automaticamente i problemi nello stile del tuo codice.
...
Ora che R ha `peso_kg` in memoria, possiamo fare aritmetica con esso. Per
esempio, potremmo voler convertire questo peso in libbre (il peso in libbre è 2,2 volte il peso in kg):
...
Questo significa che assegnare un valore ad un oggetto non cambia i valori di
altri oggetti. Per esempio, memorizziamo il peso dell'animale in libbre in un nuovo
oggetto, `peso_lb`:
...
Fino ad ora, il vostro codice è stato nella console. Questo è utile per rapide interrogazioni
ma non così utile se si vuole rivedere il proprio lavoro per qualsiasi motivo.
Uno script può essere aperto premendo <kbd>Ctrl</kbd> + <kbd>Shift</kbd> + 
<kbd>N</kbd>. 
È saggio salvare immediatamente il file dello script. Per fare questo premere 
<kbd>Ctrl</kbd> + <kbd>S</kbd>. Questo aprirà una finestra di dialogo dove 
puoi decidere dove salvare il tuo file di script e come chiamarlo.
L'estensione del file `.R` viene aggiunta automaticamente e assicura che il tuo file
si aprirà con RStudio.
...
sarà ignorato da R. È utile lasciare note e spiegazioni nei vostri
script.
...
> età <- età - 20 # età?
> mass_index <- massa/età # mass_index?
> ```
...
Vediamo che se vogliamo un numero diverso di cifre, 
possiamo digitare "cifre = 2" o quante ne vogliamo.

```{r, results = 'show', purl = FALSE}
round(3.14159, digits = 2)
```

Se fornite gli argomenti nell'esatto ordine in cui sono definiti,
non è necessario nominarli:
...
arrotondare) prima nella vostra chiamata di funzione, e poi specificare i nomi di tutti gli argomenti opzionali
opzionali. Se non lo fate, qualcuno che legge il vostro codice potrebbe dover cercare la
definizione di una funzione con argomenti non familiari per capire cosa state
facendo.
...
Un vettore è il tipo di dati più comune e basilare in R, ed è praticamente
il cavallo di battaglia di R. Un vettore è composto da una serie di valori, che possono essere
sia numeri che caratteri. Possiamo assegnare una serie di valori ad un vettore usando
la funzione `c()`. Per esempio, possiamo creare un vettore di pesi di animali e assegnarlo
ad un nuovo oggetto `peso_g`:
...
Ci sono molte funzioni che vi permettono di ispezionare il contenuto di un
vettore. La funzione `length()` vi dice quanti elementi ci sono in un particolare vettore:
...
Mentre programmiamo, questo può essere utile per aggiungere risultati che stiamo raccogliendo o
calcolando.
...
>
> ```{risposta_testo, echo=FALSE, purl=FALSE}
